var request = require('request');
var _ = require('underscore');
var tournament_sm = require('StickerMap').TournamentStickerMap;
var team_sm = require('StickerMap').TeamStickerMap;
var team_map = require('StickerMap').TeamMap;
var tournament_map = require('StickerMap').TournamentMap;
var MongoClient = require('mongodb').MongoClient;
var test = require('assert');
var collection_name = "oxlef";
var db_url = "mongodb://localhost:27017/oxlefdb";

MongoClient.connect(db_url, function (err, db) {

	//id = [stickername], full=[Boolean: get full sales history], time=[Integer: past x days avg/median summary]
	var get_data = function(id, callback, callback_params, full, time) {
		var base_api = "http://csgobackpack.net/api/GetItemPrice/";
		var params = {"currency": "USD", "id": id};

		if(full) {
			params["full"] = full;
		}
		if(time) {
			params["time"] = time;
		}
		
		//Construct api call request string
		var api_call = base_api;
		var index = 0;
		_.each(params, function(value, key) {
			if(index == 0) {
				api_call += "?";
			} else {
				api_call += "&";
			}
			api_call += key + "=" + encodeURIComponent(value);
			++index;
		});

		console.log("Making request to: " + api_call);
		request(api_call, function (error, response, body) {
			if (!error && response.statusCode == 200) {
				callback_params.unshift(JSON.parse(body));
				callback.apply(callback, callback_params);
				return true;
			}
		});
	}

	var store_data = function(data, tournament, team, sticker_type) {
			_.each(data, function(entry, index) {
				var datetime = new Date(entry[0]);
				var price = entry[1];
				var quantity = parseInt(entry[2]);

				db.collection(collection_name).update( 
					{ "team": team, "tournament": tournament, "datetime": /.*datetime.substring(0, 11).*/, "type": sticker_type},
					{ $setOnInsert:
						{
							"team": team,
							"tournament": tournament,
							"type": sticker_type,
							"datetime": datetime,
							"price": price,
							"quantity": quantity
						}
					},
					{ upsert: true },
					function(err, result) {
						test.equal(err, null);
						if(result.result.hasOwnProperty("upserted")) {
							console.log("Inserted entry: "+team+"; "+tournament+"; "+sticker_type+"; "+datetime+";");
						} else {
							console.log("Exists... Skipping: '"+team+"'; "+tournament+"; "+sticker_type+"; "+datetime+";");
						}
					}
				);
			});
	}

	var migrate_tournament = function(tournament) {
		//(stickers, sticker-type, index)
		_.each(tournament_sm[tournament], function(value, sticker_type, index) {
			_.each(value, function(sticker, team) {
				var result = JSON.parse(get_data(sticker, true));
				store_data(result, tournament, team, sticker_type);
			});
		});
	}

	var migrate_team = function(team) {
		//(stickers, sticker-type, index)
		_.each(team_sm[team], function(value, sticker_type, index) {
			if(sticker_type != "Autograph") {
				_.each(value, function(sticker, tournament) {
					get_data(sticker, store_data, [tournament, team, sticker_type], true);
				});
			} else {
				_.each(value, function(value2, sticker_type2, index) {
					_.each(value2, function(stickers, tournament) {
						_.each(stickers, function(sticker) {
							get_data(sticker, store_data, [tournament, team, sticker_type2 + " " + sticker_type], true);
						});
					});
				});
			}
		});	
	}

	var initialize_teams = function() {
		//Initialize Teams
		_.each(team_map, function(value, key, index) {
			db.collection(collection_name).update( 
				{ "_id": key },
				{ $setOnInsert:
					{
						"_id": key,
						"name": key,
						"abbreviation": value,
						"stats": {}
					},
				}, 
				{ upsert: true },
				function(err, result) {
					test.equal(err, null);
					if(result.result.hasOwnProperty("upserted")) {
						console.log("Team Inserted: '"+key+"'.");
					} else {
						console.log("Team Exists: '"+key+"'.");
					}
				}
			);
		});
	}

	var initialize_tournaments = function() {
		//Initialize Tournaments
		_.each(tournament_map, function(value, key, index) {
			db.collection(collection_name).update( 
				{ "_id": key },
				{ $setOnInsert:
					{
						"_id": key,
						"name": key,
						"fullname": value["fullname"],
						"location": value["location"],
						"prizepool": value["prizepool"],
						"start": value["start"],
						"end": value["end"],
						"stats": {}
					}
				},
				{ upsert: true },
				function(err, result) {
					test.equal(err, null);
					if(result.result.hasOwnProperty("upserted")) {
						console.log("Tournament Inserted: '"+key+"'.");
					} else {
						console.log("Tournament Exists: '"+key+"'.");
					}
				}
			);
		});
	}

	var update_team_stats = function(team) {
		var range = {"min": 0, "max": 0};		//Price range in all time
		var range_52 = {"min": 0, "max": 0};	//Price range of last 52 weeks
		var open = 0;							//Opening price
		var volume = 0; 						//Number of stickers sold over last day
		var market_cap = 0;						//Total value in the market

		db.collection(collection_name).aggregate(
			[
				{ $match: {	team: team } },
				{ 
					$group: { 
						_id: { month: { $month: "$datetime" }, day: { $dayOfMonth: "$datetime" }, year: { $year: "$datetime" } },
						volume: { $sum: "$quantity" },
						market_cap: { $sum: { $multiply: ["$price", "$quantity"] } },
						min: { $min: "$price" },
						max: { $max: "$price" },
						day_average: { $avg: "$quantity" }
					}
				},
				{ $out: "oxlef_" + team.replace(/ /g, "_") }
			]
		);

		//Mapreduce
		/*var map = function() {
	        var key = this._id;
	        var value = this.price * this.quantity;
	        var count = this.quantity;

	    	emit(key, value);
	    	emit("*", count);
	    };

	    var reduce = function(key, values) {
	    	return Array.sum(values);
	    };

	    db.collection(collection_name).mapReduce(
        	mapFunction1,
        	reduceFunction1,
        	{ out: "stats" }
        )*/

		//Update stats on team
		/*db.collection(collection_name).update(
			{"_id": team},
			{ $set: { "stats": {
					"range": range,
					"range_52": range_52,
					"open": open,
					"volume": volume,
					"avg_price": avg_price,
					"market_cap": market_cap
				}} 
			}
		);*/
		console.log(team + " stats has been updated.");
	}

	/** 
	**	Main
	**/
	if(process.argv.length < 2) {
		var found_cmd = false;
		if(process.argv[2] == "init") {
			initialize_teams();
			initialize_tournaments();
			console.log("Teams & Tournaments Initalized");
			found_cmd = true;
		} else if(process.argv[2] == "migrate") {
			if(process.argv[3] == "team") {
				migrate_team(process.argv[4]);
				console.log(process.argv[4] + " team has been migrated.");
				found_cmd = true;
			} else if (process.argv[3] == "tournament") {
				migrate_tournament(process.argv[4]);
				console.log(process.argv[4] + " tournament has been migrated.");
				found_cmd = true;
			}
		} else if(process.argv[2] == "update") {
			if(process.argv[3] == "team") {
				update_team_stats(process.argv[4]);
				console.log(process.argv[4] + " team has been updated.");
				found_cmd = true;
			} else if(process.argv[1] == "tournament") {

				console.log(process.argv[4] + " tournament has been migrated.");
				found_cmd = true;
			}
		} 

		if(!found_cmd) {
			console.log("Unrecognized command.");
		}
	} else {
		console.log("Please enter argument(s) for one of following commands: ");
		console.log("\t[init]");
		console.log("\t[migrate team 'TeamName']");
		console.log("\t[migrate tournament 'TournamentName']");
		console.log("\t[update team 'TeamName']");
		console.log("\t[update tournament 'TournamentName']");
	}

	process.exit(0);
});